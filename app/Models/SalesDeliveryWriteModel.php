...
final class SalesDeliveryWriteModel extends Model
{
    ...
    public function createDraftDelivery(array $data): array
    {
        ...
        if (! $order) {
            throw new RuntimeException('Sales Order tidak ditemukan atau belum siap delivery pada company aktif.');
        }
        ...
        foreach ($this->normalizeLines($data) as $line) {
            ...
            $preparedLines[] = ['so_item' => $item, 'qty_delivered' => $qtyDelivered, 'unit_price' => (float) $item->unit_price, 'line_total' => $lineTotal];
    ...
    public function postDelivery(int $deliveryId, int $companyId, int $userId): array
    {
        ...
        foreach ($items as $item) {
            ...
            $this->db->table('sales_order_items')->where('id', (int) $soItem->id)->update([
            ...
            $this->db->table('delivery_order_items')->where('id', (int) $item->id)->update([
                'status' => 'posted',
                'updated_by' => $userId,
                'updated_at' => $now,
            ]);
        ...
        $this->updateSalesOrderDeliveryStatus((int) $delivery->sales_order_id, $companyId, $userId, $now);

        $this->db->table('delivery_orders')->where('id', (int) $delivery->id)->update([
            'status' => 'posted',
            'updated_by' => $userId,
            'updated_at' => $now,
        ]);
    ...
    private function updateSalesOrderDeliveryStatus(int $salesOrderId, int $companyId, int $userId, string $now): void
    {
        $summary = $this->db->table('sales_order_items')
            ->select('SUM(qty_remaining) AS remaining_qty, SUM(qty_delivered) AS delivered_qty')
        ...
        $delivered = (float) ($summary->delivered_qty ?? 0);
        $status = $remaining <= 0.00001 ? 'delivered' : ($delivered > 0 ? 'partial_delivered' : 'confirmed');
    ...
    {
        $last = $this->db->table('delivery_orders')->selectMax('id', 'max_id')->where('company_id', $companyId)->get()->getRow();
