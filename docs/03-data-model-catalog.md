...
| `audit_logs` (immutable action trail) | `id PK`, `user_id FK users NULL`, `event_type VARCHAR(80)`, `entity_type VARCHAR(80)`, `entity_id BIGINT NULL`, `request_id CHAR(36)`, `ip_address VARCHAR(45)`, `before_hash CHAR(64)`, `after_json JSON`, `occurred_at DATETIME` | Append-only; `IDX(company_id,occurred_at)`, `IDX(entity_type,entity_id)`; no update/delete; event password hanya menyimpan metadata non-rahasia | `USER_PROVISIONED`, `USER_STATUS_UPDATED`, `USER_PASSWORD_REPLACED`, `USER_COMPANY_MEMBERSHIP_UPDATED` |
