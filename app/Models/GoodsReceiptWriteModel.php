...
final class GoodsReceiptWriteModel extends Model
{
    ...
    public function createDraftReceipt(array $data): array
    {
        ...
        if (! $po) {
            throw new RuntimeException('Purchase Order tidak ditemukan atau belum confirmed pada company aktif.');
        }
        ...
        foreach ($this->normalizeReceiptLines($data) as $line) {
            ...
            $productExists = $this->db->table('products')
            ...
            $preparedLines[] = ['po_item' => $item, 'qty_received' => $qtyReceived, 'unit_cost' => (float) $item->unit_price, 'line_total' => $lineTotal];
            $totalQty += $qtyReceived;
            $totalAmount += $lineTotal;
        }

        if ($preparedLines === []) {
            throw new RuntimeException('Minimal satu item harus memiliki qty diterima lebih dari nol.');
        }
    ...
    public function postReceipt(int $receiptId, int $companyId, int $userId): array
    {
        ...
        foreach ($items as $item) {
            ...
            $this->db->table('purchase_order_items')->where('id', (int) $poItem->id)->update([
            ...
            if ($balance) {
                $this->db->table('stock_balances')->where('id', (int) $balance->id)->update([
            ...
            $this->db->table('goods_receipt_items')->where('id', (int) $item->id)->update([
                'status' => 'posted',
                'updated_by' => $userId,
                'updated_at' => $now,
            ]);
        ...
        $this->updatePurchaseOrderReceiptStatus((int) $receipt->purchase_order_id, $companyId, $userId, $now);

        $this->db->table('goods_receipts')->where('id', (int) $receipt->id)->update([
            'status' => 'posted',
            'updated_by' => $userId,
            'updated_at' => $now,
        ]);
    ...
    private function normalizeReceiptLines(array $data): array
    {
        ...
        if (is_array($posted)) {
            ...
            foreach ($posted as $item) {
                ...
                $lines[] = ['purchase_order_item_id' => $poItemId, 'qty_received' => $qty];
            }
            return $lines;
        }

        $poItemId = (int) ($data['purchase_order_item_id'] ?? 0);
        $qty = $this->decimal($data['qty_received'] ?? 0);
        return ($poItemId > 0 && $qty > 0) ? [['purchase_order_item_id' => $poItemId, 'qty_received' => $qty]] : [];
    }

    protected function nextReceiptNumber(int $companyId): string
    {
        $last = $this->db->table('goods_receipts')->selectMax('id', 'max_id')->where('company_id', $companyId)->get()->getRow();
        return 'GR' . date('Ym') . '-' . str_pad((string) (((int) ($last->max_id ?? 0)) + 1), 4, '0', STR_PAD_LEFT);
    }

    private function updatePurchaseOrderReceiptStatus(int $purchaseOrderId, int $companyId, int $userId, string $now): void
    {
        $summary = $this->db->table('purchase_order_items')
            ->select('SUM(qty_remaining) AS remaining_qty, SUM(received_qty) AS received_qty')
            ->where('purchase_order_id', $purchaseOrderId)
            ->where('company_id', $companyId)
            ->where('deleted_at IS NULL', null, false)
            ->get()->getRow();

        $remaining = (float) ($summary->remaining_qty ?? 0);
        $received = (float) ($summary->received_qty ?? 0);
        $status = $remaining <= 0.00001 ? 'received' : ($received > 0 ? 'partial_received' : 'confirmed');

        $this->db->table('purchase_orders')->where('id', $purchaseOrderId)->where('company_id', $companyId)->update([
            'status' => $status,
            'updated_by' => $userId,
            'updated_at' => $now,
        ]);
    ...
    {
        (new AuditTrailService($this->db))->record($event, 'goods_receipt', $entityId, $companyId, $branchId, $userId, $after);
    }
