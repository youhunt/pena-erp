...
| `purchase_orders` (vendor commitment) | `id PK`, `branch_id FK`, `supplier_id FK`, `warehouse_id FK`, `currency_id FK`, `term_id FK NULL`, `transaction_code_id FK`, `po_no VARCHAR(50)`, `order_date DATE`, `expected_receipt_date DATE NULL`, `supplier_ref_no VARCHAR(80) NULL`, `subtotal DECIMAL(19,4)`, `tax_amount DECIMAL(19,4)`, `total_amount DECIMAL(19,4)`, `status VARCHAR(30)`, `document_upload_id FK NULL` | `UQ(company_id,po_no)`, `IDX(company_id,supplier_id,status)` | `JKT-PO-000001` |
| `purchase_order_items` (PO lines) | `id PK`, `purchase_order_id FK`, `product_id FK`, `qty DECIMAL(18,4)`, `received_qty DECIMAL(18,4)`, `uom_id FK`, `unit_price DECIMAL(19,4)`, `tax_code_id FK NULL`, `tax_rate DECIMAL(9,6)`, `tax_amount DECIMAL(19,4)`, `line_total DECIMAL(19,4)` | `IDX(company_id,purchase_order_id)` | `A4 10 x125000` |
...
| `sales_orders` (customer order) | `id PK`, `branch_id FK`, `customer_id FK`, `warehouse_id FK`, `currency_id FK`, `term_id FK NULL`, `transaction_code_id FK`, `order_no VARCHAR(50)`, `order_date DATE`, `requested_ship_date DATE NULL`, `customer_po_no VARCHAR(80) NULL`, `subtotal DECIMAL(19,4)`, `tax_amount DECIMAL(19,4)`, `total_amount DECIMAL(19,4)`, `status VARCHAR(30)`, `document_upload_id FK NULL` | `UQ(company_id,order_no)`, `IDX(company_id,customer_id,status)` | `JKT-SO-000001` |
| `sales_order_items` (order lines) | `id PK`, `sales_order_id FK`, `product_id FK`, `qty DECIMAL(18,4)`, `reserved_qty DECIMAL(18,4)`, `delivered_qty DECIMAL(18,4)`, `uom_id FK`, `unit_price DECIMAL(19,4)`, `tax_code_id FK NULL`, `tax_rate DECIMAL(9,6)`, `tax_amount DECIMAL(19,4)`, `line_total DECIMAL(19,4)` | `IDX(company_id,sales_order_id)` | `A4 2` |
...
| `journal_entries` (ledger header) | `id PK`, `gl_book_id FK`, `journal_no VARCHAR(50)`, `journal_date DATE`, `source_type VARCHAR(40)`, `source_id BIGINT NULL`, `description VARCHAR(200)`, `status VARCHAR(20)`, `posted_at DATETIME NULL`, `posted_by FK NULL`, `reversal_of_id FK NULL` | `UQ(company_id,journal_no)`, `IDX(company_id,journal_date,status)`, source index; posting checks fiscal/module `gl` open | `JV-20260528-0001 posted` |
| `journal_entry_lines` (debit/credit) | `id PK`, `journal_entry_id FK`, `account_id FK`, `description VARCHAR(200)`, `debit DECIMAL(19,4)`, `credit DECIMAL(19,4)`, `partner_type VARCHAR(20) NULL`, `partner_id BIGINT NULL` | `IDX(company_id,journal_entry_id)`, `IDX(company_id,account_id)`; debit total must equal credit total | `AP credit 1,387,500` |
