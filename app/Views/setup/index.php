    ...
    <div class="card-body">
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane fade" id="tab-department">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        <thead><tr><th>Site</th><th>Code</th><th>Department Name</th><th>Status</th><?php if ($canManage) : ?><th class="text-end">Action</th><?php endif; ?></tr></thead>
                        <tbody>
                        <?php foreach ($departments as $department) : ?><tr>
                            <td><?= esc($department['branch_code'] ?? '-') ?></td><td><strong><?= esc($department['code']) ?></strong></td><td><?= esc($department['name']) ?></td><td><?= $statusBadge((string) $department['status']) ?></td>
                            <?php if ($canManage) : ?><td class="text-end text-nowrap">
                                <button class="btn btn-sm btn-outline-primary js-edit-department" data-bs-toggle="modal" data-bs-target="#editDepartment" data-id="<?= (int) $department['id'] ?>" data-site="<?= esc($department['branch_code'] ?? '-', 'attr') ?>" data-code="<?= esc($department['code'], 'attr') ?>" data-name="<?= esc($department['name'], 'attr') ?>">Edit</button>
                        ...
                        <?php if ($departments === []) : ?><tr><td colspan="<?= $canManage ? 5 : 4 ?>" class="text-center text-muted py-4">Belum ada department.</td></tr><?php endif; ?>
                        </tbody>
                    </table>
                </div>
            </div>
...
document.querySelectorAll('.js-edit-department').forEach(function (button) {
    button.addEventListener('click', function () {
        ...
        form.querySelector('[data-field="site"]').value = button.dataset.site;
